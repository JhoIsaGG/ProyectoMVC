...
<body>
    ...
    <div class="mainContainer user-management-container" style="margin-top: 100px;">
        <h2 class="user-management-title">Gestión de Niveles</h2>
        
        <div class="actions-bar">
            <a class="btn btn-create-user" href="index.php?action=nivel_new">+ Crear nuevo</a>
        </div>
                    ...
                    <?php if (!empty($items)): ?>
                        <?php foreach ($items as $item): ?>
                            <tr class="element-user">
                                ...
                                <td>
                                    <?php if ($item['estado'] == 1): ?>
                                        <a class="btn btn-edit-sm" href="index.php?action=nivel_edit&codigo=<?php echo urlencode($item['id_nivel']); ?>">Editar</a>
                                        <form target="fakeFrame" action="index.php?action=nivel_delete" method="POST" style="display:inline;">
                                            <input type="hidden" name="codigo" value="<?php echo htmlspecialchars($item['id_nivel']); ?>">
                                            <button class="btn btn-delete-sm" type="submit" onclick="return confirmacionEliminar();">Eliminar</button>
                                        </form>
                                    <?php else: ?>
                                        <form target="fakeFrame" action="index.php?action=nivel_reactivate" method="POST" style="display:inline;">
                                            <input type="hidden" name="codigo" value="<?php echo htmlspecialchars($item['id_nivel']); ?>">
